...
1. **会话开始 / 收到任务后第一件事**：感知记忆系统。
   - SessionStart hooks 已自动注入记忆概览（最近 8 条 + 统计），把它当上下文事实使用。
   - 未注入时主动调用 `cerebrate_sense`。
2. **任务开工前（需求调研阶段）**：先查历史记忆 —— 这是证据收集的一部分。
   - 用 `cerebrate_search(query, scope=project/all)` 检索；项目相关任务必须传
     `project_id`（避免通用记忆隔离误伤）。
   - 命中后用 `cerebrate_timeline`（前因后果）与 `cerebrate_detail`（完整详情）补齐。
   - 无命中 ≠ 不存在：换关键词/换 scope 再查一次，仍无再判断"新经验"。
3. **任务过程中**："之前可能解决过类似问题"的判断，先查记忆再回答/动手。
4. **任务完成后**：有可复用经验/教训时主动 `cerebrate_propose` 提交，让记忆生长。
5. **记忆即证据**：检索结果与日志/数据库/代码同等地位，不能跳过记忆查询凭经验臆断。

记忆系统不可用时降级为无记忆工作，但要在总结中说明。
